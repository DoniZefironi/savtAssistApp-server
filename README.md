...
>
> Если фронтенд всегда обращается к этим путям с префиксом `/backend/...` (независимо от того, каким портом/server-блоком он сейчас обслуживается), обход нужен **в каждом** server-блоке, который отдаёт фронтенд — не только в `listen 8080`. Изначально `location /backend/operator/events/`/`location /backend/user-events/` (со снятием префикса через `rewrite ... break;`) были прописаны только там; отсутствие такого же обхода в `listen 443 ssl` (в обоих `nginx.conf` и `nginx.http-fallback.conf`) было реальной причиной, почему SSE не доходил до браузера в проде — запрос падал в общий `location /`, летел на фронтенд, и уже там (не на уровне nginx) буферизовался `rewrites()`. Теперь такой же обход есть во всех server-блоках, что отдают фронтенд (443 и 8080) — обычные (не-realtime) `/backend/*`-запросы по-прежнему идут через фронтендовый rewrite как раньше, трогать `next.config.js` не нужно.
