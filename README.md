...
> (`channel: "telegram"`, других значений схема не примет).
>
> **Владение номером подтверждается.** Раньше код просто улетал в тот Telegram,
> который открыл ссылку, а введённый в форме номер не сверялся ни с чем — занять
> можно было любой незарегистрированный номер. Теперь бот просит поделиться
> контактом: Telegram сам подставляет номер, который проверил при регистрации
> аккаунта, и мы сверяем его с заявленным. Ввести номер руками в боте нельзя.
>
> Для фронта это значит: между «открыл бота» и «пришёл код» появился ещё один
> шаг, который пользователь делает **внутри Telegram** — нажимает кнопку
> «Отправить мой номер».
...
1. POST /auth/register/start
   ...
   deep_link != null → обычный случай для новой регистрации.
     → Показать кнопку "Открыть Telegram" (открыть deep_link).
     → В Telegram пользователь жмёт Start, затем кнопку "Отправить мой номер".
     → Только после этого бот присылает код.
   deep_link == null → Telegram у этого аккаунта уже подключён и подтверждён
     ранее, код уже отправлен. → Сразу экран ввода кода.

   Что видит пользователь в Telegram, если номер не тот:
     "Номер вашего Telegram не совпадает с указанным при регистрации (+375...)"
     → Ему надо вернуться в приложение и начать заново с номером этого
       Telegram-аккаунта. Заявка при этом не сгорает.
...
### Мобильное приложение (пользователь)
- [ ] **Выбор канала больше не нужен** — Viber удалён. Поле `channel` осталось в теле запросов ради совместимости, но принимает только `"telegram"`; любое другое значение → `422`. Если в UI был переключатель Telegram/Viber — убрать.
- [ ] Состояние экрана при непустом `deep_link`: кнопка **«Открыть Telegram»**, а не поле ввода кода. Код в этот момент ещё не существует.
- [ ] **Объяснить пользователю второй шаг внутри Telegram.** После Start бот покажет кнопку «Отправить мой номер» — без неё код не придёт. Текст вида: «Откройте Telegram, нажмите Start, затем кнопку "Отправить мой номер"». Это самое частое место, где человек застрянет, если не предупредить.
- [ ] Обработать случай несовпадения номера: пользователь указал в форме один номер, а Telegram-аккаунт зарегистрирован на другой. Бот напишет об этом в чат, но приложение будет молча ждать код — стоит дать подсказку «код не приходит? проверьте, что номер совпадает с номером вашего Telegram» и кнопку «изменить номер».
- [ ] Способ узнать, что код отправлен — как и раньше, отдельного сигнала нет: проще всего дать экран ввода кода, когда пользователь вернулся в приложение.
- [ ] Если `deep_link: null` — Telegram уже подтверждён ранее, сразу показывать поле ввода кода.
- [ ] **Сброс пароля:** `deep_link` теперь всегда `null`. Если код не пришёл — значит у аккаунта нет подтверждённой связки с ботом, и самостоятельно это уже не чинится. Показывать «обратитесь к администратору», а не кнопку открытия бота.
...
Telegram-ботом (SMS отключено полностью, Viber удалён).
```json
{
  "phone": "+375291234567",
  "password": "minLength8",
...
- `channel` — только `telegram` (Viber удалён, другое значение — `422`)
...
- `deep_link` — `null`, если Telegram у аккаунта уже подключён и подтверждён ранее:
  код уже отправлен в чат с ботом. Иначе приходит ссылка `https://t.me/<bot>?start=<token>`.

**Подтверждение владения номером (важно).** Открыть ссылку недостаточно — она лишь
начинает диалог. Дальше бот показывает кнопку «Отправить мой номер»
(`request_contact`), и код генерируется, только если Telegram прислал номер,
совпадающий с полем `phone` этого запроса.

Проверяется три вещи, и все три обязательны:
1. контакт принадлежит отправителю (`contact.user_id == message.from.id`) — Telegram
   разрешает переслать боту **любой** контакт из адресной книги, без этой проверки
   достаточно было бы отправить карточку жертвы;
2. номер совпадает с заявленным при регистрации;
3. заявка не просрочена и не использована.

Пока номер не подтверждён, код не создаётся вовсе. До этой правки код улетал любому,
кто открыл ссылку, и заявленный номер не сверялся ни с чем — то есть занять можно
было любой незарегистрированный номер, не имея к нему доступа.
