...
Исключение: рассылка админа (POST /admin/notifications/broadcast)
переключатель promotional не проверяет и уходит всем —
он выключен по умолчанию, и фильтрация обрубила бы рассылки почти всем.
```
...
|---|---|---|
| `phone_already_registered` | этот Telegram уже привязан к аккаунту | «Войдите по этому номеру» + кнопки «Войти» / «Забыли пароль» |
...
> снимется сама. При `phone_already_registered` повтор не поможет — Telegram отдаст
> тот же номер.
...
если разошлось название, докладывает зеркала документов — и **подхватывает файлы,
положенные в папку напрямую** (см. ниже).
