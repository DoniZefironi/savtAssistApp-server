...
> Идемпотентности через `client_token` тут нет — она и не нужна: повторный
> вызов с тем же кодом от того же пользователя и так не создаёт дубль, а
> закономерно отвечает `409` (уже привязан / заявка уже отправлена). Для
> оффлайн-очереди на клиенте это и есть сигнал «уже синхронизировано» —
> `409` на этой ручке нужно трактовать как успех, а не как ошибку, и снимать
> действие из очереди.
