...
`GET /kb/*` и `GET /faq/*` (см. `POST /auth/guest` в разделе ниже). Хранить
гостевой `access_token` в памяти, не в Keychain/Keystore — это не аккаунт, и
"запоминать" его незачем: закончился — просто запросить новый.
