...
→ Показать галерею. url — подписанная ссылка, см. «Подписанные ссылки на файлы».
→ 403, если ШУ не привязан к аккаунту (раньше проверки не было — фото любого ШУ
  отдавались любому авторизованному пользователю).
```
...
Ответ: { items: [{ id, project_id, url, caption, sort_order }], ... }
→ 403, если проект не привязан к аккаунту.
```
Отдельный список от фото ШУ — как и с документами. У фото заполнено **ровно одно**
из `cabinet_id` / `project_id`.
...
Проверка настроек живёт в send_push, поэтому действует
...
a835bebb7d46). В журнал действий пишется, скольким разослали
и сколько отписавшихся пропущено.
```
...
При создании сервер автоматически заводит на NAS (если настроен `PROJECT_FOLDERS_ROOT`, см. переменные окружения) папку с шаблонной структурой — внутри **годовой папки**, как разложено в Bitrix:
```
...
**Год берётся из производственного номера**: `26_170` → `!2026`, `25_003` → `!2025`. У проектов, заведённых вручную через админку, номера нет — для них берётся год создания записи, чтобы в корне не появлялось исключений и он оставался только из годовых папок. Восклицательный знак — чтобы годовые папки всплывали над остальным при сортировке по имени.
...
`warranty_status` считается по гарантии **самого проекта**, а не его шкафов: `active` / `expiring_soon` (≤30 дней) / `expired` / `none`.
...
докладывает зеркала документов — и **подхватывает файлы, положенные в папку
напрямую** (см. ниже).
...
`Фото`, `Переписка` и т.п.) не трогаются: файлы там разложены осмысленно, и
...
Переименование, смена родителя и гарантия. Все поля опциональны — передавать только изменённые.
```json
{
  "name": "Новое название",
  "parent_project_id": null,
...
- **Гарантия** — единственное, что администратор задаёт руками: в CRM такого поля нет.
...
### POST `/admin/projects/{project_id}/sync-folder`
Ручной запуск синхронизации папки проекта на NAS — работает всегда (в отличие от ночного автопрогона, не проверяет актуальность гарантии). Проверяет/чинит структуру подпапок, переименовывает корень при рассинхроне с `name`, переносит папку в её годовую (`!2026`), если она ещё лежит плоско в корне, докопировает в корень отсутствующие файлы документов проекта, восстанавливает `_Маркировка/QR.png`, если он отсутствует. Ответ — `ProjectOut` с обновлённым `folder_synced_at`.
