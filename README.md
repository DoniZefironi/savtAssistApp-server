...
Исходящий вебхук Bitrix24 на событие `ONTASKCOMMENTADD` (добавление комментария к задаче). Пересылает в чат заявки в приложении только комментарии, начинающиеся с префикса `_FORWARD_PREFIX` (см. `app/services/bitrix_webhook_service.py`, сейчас `/sa`) — остальная переписка в задаче остаётся только в Bitrix. Сам вебхук несёт только `TASK_ID`/`COMMENT_ID` — текст комментария дополнительно дотягивается через `task.commentitem.get`.

> Если панель «Чат» у задачи в Bitrix — это IM-модуль, а не классические комментарии, префикс стоит выбирать так, чтобы он не совпадал со слэш-командами/упоминаниями, которые сам Bitrix может перехватывать на клиенте до отправки (например, `/all` совпадает с командой «уведомить всех» и может не долетать до вебхука как обычный текст).
