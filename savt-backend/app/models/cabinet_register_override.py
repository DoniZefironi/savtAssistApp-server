...
class CabinetRegisterOverride(Base):
    ...
    для конкретного ШУ — либо новый (адрес, бит), которого нет в стандартной
    карте, либо другое название уже существующего (адреса, бита) именно для
    этого ШУ. При расшифровке сначала смотрим сюда, и только если тут нет
    строки на нужный (адрес, бит) — берём из RegisterDefinition."""
