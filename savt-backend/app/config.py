...
class Settings(BaseSettings):
    ...
    # Телеметрия ШУ (аварии/сообщения с MQTT-контроллеров) — приходит не с самого
    # контроллера, а от промежуточного прокси на C#, который слушает MQTT и шлёт
    # уже сюда обычным HTTP. Секрет — общий, в заголовке X-Telemetry-Secret.
    telemetry_webhook_secret: str = ""
    # Сколько дней хранить сырую историю сообщений телеметрии (для аудита/разбора
    # задним числом, см. GET /admin/cabinets/{id}/telemetry/history) — старше
    # удаляется фоновой задачей (см. app/main.py). "Текущее состояние" регистров
    # (что видит обычная лента) от этого не зависит, оно не история и не растёт.
