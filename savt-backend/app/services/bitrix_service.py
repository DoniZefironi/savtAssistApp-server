...
# Стадии, на которых Bitrix требует служебное поле и дедлайн. На "Новой" и
# "На рассмотрении" не требует ничего — и слать туда эти поля незачем:
# дедлайн пришлось бы выдумывать на пустом месте, а служебное поле вообще
# команда для бизнес-процесса, которой на этих стадиях делать нечего
_STATUSES_WITH_REQUIRED_FIELDS = ("in_progress",) + _CLOSING_STATUSES
...
async def update_reclamation_stage(
    ...
    ПОЧЕМУ in_progress НЕ ОТПРАВЛЯЕТСЯ. На стороне портала на стадии CLIENT
    висит робот, который сам доводит карточку до SUCCESS. Доказано 2026-09-23
    на карточке 71: stageId SUCCESS, previousStageId CLIENT, movedBy=0 —
    ноль означает автоматизацию, наш вебхук ходит под 261, у сотрудника был бы
    свой ID. Срабатывает НЕ мгновенно, с задержкой в минуты — из-за чего
    проверка сразу после перевода показывает CLIENT и создаёт ложное
    впечатление, что всё исправно (мы на это уже попались).

    Вред не только в закрытой карточке: робот двигает стадию, стадия приезжает
    к нам вебхуком как resolved, и заявителю уходит пуш "Рекламация исполнена"
    по заявке, которую только взяли в работу.

    Обойти из кода нельзя: без служебного поля Bitrix не пускает, с ним
    срабатывает робот. Чинится только на портале — робот не должен добивать
    карточку до закрытия. Когда починят, снять блок ниже (одна строка) и
    проверить не через десять секунд, а минут через десять.
    ...
    if status == "in_progress":
        _log.warning(
            "Bitrix reclamation item %s: перевод в работу не отправлен — на стадии "
            "CLIENT робот портала сам добивает карточку до SUCCESS (см. докстринг)",
            item_id,
        )
        return None
    ...
    elif needs_required_fields:
        # Дедлайна нет, а стадия его требует. Не роняем переход из-за пустого
        # поля, но и не затираем значение, если его успели проставить в Bitrix.
        # На стадиях, где дедлайн не обязателен, выдумывать его не надо — там
        # пустое поле никому не мешает
...
async def create_reclamation_item(
        ...
        object_serial_number: str | None = None, contract_info: str | None = None,
        component_info: str | None = None,
) -> str | None:
    ...
    object_serial_number/contract_info/component_info — с 2026-09-23 в процессе
    появились отдельные нативные поля под эти данные (раньше шли только текстом
    внутри sourceDescription) — заполняем их, когда есть что подставить,
    сборка значений на стороне вызывающего (см. reclamation_service._sync_to_bitrix)."""
    if not settings.bitrix_webhook_url:
        return None
    ...
    fields = {
        ...
        "stageId": RECLAMATION_INITIAL_STAGE,
    }
    if deal_id:
    ...
    if object_serial_number:
        fields["ufCrm53_1789991563886"] = object_serial_number
    if contract_info:
        fields["ufCrm53_1789991629716"] = contract_info
    if component_info:
        fields["ufCrm53_1789991882464"] = component_info
