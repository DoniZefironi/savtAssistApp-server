...
# Исходящий вебхук Bitrix24 на событие ONTASKUPDATE — мгновенная синхронизация
# статуса задачи в заявку (доп. к плановому опросу раз в 15 минут, см. README)
@router.post("/task-update", status_code=status.HTTP_204_NO_CONTENT)
async def task_update_webhook(request: Request, background_tasks: BackgroundTasks):
    form = dict(await request.form())
    if not verify_token(form):
        raise HTTPException(status_code=403, detail="Invalid application_token")
    background_tasks.add_task(handle_task_update_webhook, form)
