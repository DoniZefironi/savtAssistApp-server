...
router = APIRouter(prefix="/admin/project-requests", tags=["admin: project requests"])

# Все заявки на вступление в проект
@router.get("", response_model=PageOut[ProjectShareRequestOut])
async def list_shares(
    status: str | None = Query(None, pattern="^(pending|approved|rejected)$"),
    resolved_by_admin_id: int | None = Query(None, gt=0),
    search: str | None = Query(None, min_length=1, max_length=200),
    sort_by: str = Query("created_at", pattern="^(created_at|resolved_at|status|user_full_name|project_name)$"),
    ...
    return await ProjectRequestService(session).list_shares(
...
async def approve_share(
    ...
    await ProjectRequestService(session).approve_share(request_id, payload, actor.id, actor_role)
...
async def reject_share(
    ...
    await ProjectRequestService(session).reject_share(request_id, payload, actor.id, actor_role)
